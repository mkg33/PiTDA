...
while counter_1 < 5:
    ...
    while r_projection - l_projection > 1e-6:
        ...
        cover = mp.CubicalCover(n_intervals=3, overlap_frac=m_projection) # Define cover
        ...
        g = graph
        num_connected_components = len(g.connected_components())
        if  g.are_connected(g.vs[0], g.vs[1]) and g.are_connected(g.vs[1], g.vs[2]) and g.are_connected(g.vs[2], g.vs[0]):
            r_projection = m_projection
        else:
            l_projection = m_projection
        
    print(g.are_connected(g.vs[0], g.vs[1]))
