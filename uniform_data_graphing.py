...
while counter_1 < 1000:
    ...
    data_y = []
    counter_2 = 0
    interval = np.random.uniform(1,100)
    data_x = np.random.uniform(1,100,20)

    while counter_2 < 20:
        y = 0
        new_y = 0 + counter_2*interval
        data_y.append(new_y)
        counter_2 += 1
...
plt.title("Line of Uniform Points vs Minimum Overlap, cover intervals = 15")
plt.show()
...
saved_file.to_csv("line_data.csv", index = False)